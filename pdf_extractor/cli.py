...
from pdf_extractor.config import DEFAULT, ExtractorConfig
...
def run(pdf, example, example_file, out_dir, pages, batch, dpi, api_key, skip_analyze):
    ...
    if not skip_analyze:
        step("分析书籍结构（抽样页面）")
        from pdf_extractor.analyzer import analyze_book
    ...
    step("生成提取 Prompt")
    from pdf_extractor.prompt_builder import build_extraction_prompt
    ...
    from pdf_extractor.extractor import extract_pages
    ex = extract_pages(
    ...
    # 5. 合并去重
    step("合并去重")
    from pdf_extractor.merger import merge_windows
