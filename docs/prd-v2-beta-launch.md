...
> 定位复盘：本产品是「量化求职者的面试刷题平台」。对照 发现→学习→巩固→检验→反馈 的用户旅程，
> M1-M4 覆盖了基础设施与检验环节，但「每日惯性、深度加工、巩固效率」三个留存关键环节缺失，M5 补齐。

#### M5-1 每日一题

- 全站每天推荐同一道题（按日期确定性选取），制造讨论一致性与打卡惯性
- 首页顶部「每日一题」卡片，完成后显示 ✅ 与连续天数（复用 streak）
- 后端：`GET /api/questions/daily` — 日期 hash 确定性选题，无需新表

#### M5-2 题目笔记

- 详情页可写个人笔记（思路、易错点、公式记忆法），匿名按设备、登录后跨设备
- 后端：`notes` 表 + `GET/PUT /api/questions/{id}/note`（空内容即删除）
- 面试备考的核心加工行为，也是后续「导出复习笔记」的数据基础

#### M5-3 智能刷题

- 「随机一题」升级智能模式：优先出**最近答错**的题，其次**未做过**的题，最后才全随机
- 后端：`/api/questions/random?mode=smart` — 三级回退选题
- 轻量间隔重复，不引入复杂 SR 算法

#### M5-4 已掌握标记

- 错题本每题可手动标记「已掌握」，移出错题本（重新答错自动回归）
- 后端：`mastered_questions` 表 + toggle 接口；错题本查询排除已掌握
- 补全 M3-1 预留的掌握闭环

**验收标准**：
- 同一天所有用户的每日一题相同，次日更换
- 笔记保存后刷新/换页仍在；登录用户换设备可见
- 智能刷题在有错题时 100% 优先出错题
- 标记已掌握后错题本立即移出，再次答错自动回归
...
| # | 任务 | 说明 | 工时 |
|---|------|------|------|
| E01 | 每日一题 API + 首页卡片 | 日期确定性选题 | 0.5d |
| E02 | 笔记表 + CRUD + 详情页笔记区 | 匿名/登录双身份 | 1d |
| E03 | 智能刷题（错题>未做>随机） | random 接口加 mode 参数 | 0.5d |
| E04 | 已掌握标记 + 错题本联动 | toggle + 查询排除 | 0.5d |
