...
> 现有 MCQ 变体由 LLM 批量生成，部分干扰项过于明显（数量级差异大、与题干无关），削弱测验效果。

- 质检维度：干扰项与正确答案同量级、同单位、形式一致（都是分数/都是小数）
- 常见错误答案作干扰项（如概率题用「未除以 2」「漏乘组合数」的典型错误结果）
- 实施：批量扫描现有 choice 题 → LLM 评分（1-5 迷惑度）→ 低分项重新生成 → 抽样人工复核
- 复用现有 quant-qa-reviewer / quant-transformer agent 流程

#### M4-6 题目双语切换（原文 / 中文翻译）

> 题库源书均为英文，部分用户读题慢影响刷题效率；但量化面试实战是英文，需保留原文。

- 数据：questions 表新增 `stem_zh`、`answer_zh` 列（可空）；LLM 批量翻译，保留 LaTeX 公式与专业术语（术语给出中英对照）
- 后端：`QuestionDetail` 返回双语字段
- 前端：详情页题干区右上角「EN/中」切换按钮，偏好写入 settings store 持久化（全局默认语言 + 单题临时切换）
- 分批翻译：优先 published 高频题，翻译状态可在看板追踪

**验收标准**：
- 筛选「概率 × 选择题 × P3」能正确过滤
- 搜索「Black-Scholes」返回相关题目，无噪音
- 完成一次模考并看到成绩报告
- 抽样 50 道选择题，干扰项迷惑度评分 ≥3 的占比 90%+
- 双语切换即点即换，公式渲染无损，偏好跨页面保持
...
| A02 | 手机号验证码 API | Provider 可插拔：Beta 用 dev 模式（码在日志/响应），ICP 备案后切腾讯云短信 | 1d |
...
| D05 | 干扰选项质检 + 重生成 | LLM 评分迷惑度，低分重生成 | 1.5d |
| D06 | 题目中文翻译入库 | stem_zh/answer_zh 批量翻译 | 1.5d |
| D07 | 前端双语切换 | EN/中 按钮 + 偏好持久化 | 0.5d |
